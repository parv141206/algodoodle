...
const convertToRecursionNode = (node: TreeNode | undefined, highlightVal: number | undefined): RecursionNode | undefined => {
  ...
  if (node.left) {
      ...
      // dummy left node to preserve shape
  ...
  if (node.right) {
      children.push(convertToRecursionNode(node.right, highlightVal)!);
       ...
       // dummy right node to preserve shape
...
const Page: FC = () => {
  ...
  const inorder = useMemo(() => {
    ...
    const findInorder = (current: TreeNode) => {
      if (current.left) {
        findInorder(current.left);
      }
      result.push(current);
      if (current.right) {
        findInorder(current.right);
      }
    };
    findInorder(tree);
    return result;
  ...
  const currentHighlight = inorder[index]?.value;
  ...
  return (
    <div>
      ...
      <AlgorithmCodeBlock algorithmName="data_structures/tree/inorder" />
    </div>
  );
};
